  ...
  return _createClass(ActionsService, null, [{
    ...
    value: function renderShareItems() {
      ...
      _Helpers_Helpers__WEBPACK_IMPORTED_MODULE_2__.Helpers.each(_Helpers_Helpers__WEBPACK_IMPORTED_MODULE_2__.Helpers.getSelectedItems(), function (value) {
        ...
        switch (shareType) {
          case 'html':
            results.push(value.type === 'image' ? "<img src=\"".concat(imageUrl, "\" alt=\"").concat(value.alt, "\" />") : "<a href=\"".concat(imageUrl, "\" target=\"_blank\">").concat(value.alt, "</a>"));
            break;
          case 'markdown':
            results.push((value.type === 'image' ? '!' : '') + "[".concat(value.alt, "](").concat(imageUrl, ")"));
            break;
          case 'indirect_url':
    ...
    value: function handleDownload(files) {
      ...
      $httpClient.make().withResponseType('blob').post(RV_MEDIA_URL.download, {
        ...
        if (contentDisposition) {
          var fileNameMatch = contentDisposition.match(/filename\*=UTF-8''(.+)|filename="?([^";\n]+)"?/i);
          if (fileNameMatch) {
  ...
  return _createClass(EditorService, null, [{
    ...
    value: function editorSelectFile(selectedFiles) {
      ...
      if (window.opener && is_ckeditor) {
        ...
        var imageUrl = firstItem.full_url;
        if (imageUrl && imageUrl.startsWith(window.location.origin)) {
          imageUrl = imageUrl.replace(window.location.origin, '');
        }
        window.opener.CKEDITOR.tools.callFunction(_App_Helpers_Helpers__WEBPACK_IMPORTED_MODULE_0__.Helpers.getUrlParam('CKEditorFuncNum'), imageUrl);
        if (window.opener) {
          window.close();
        }
      } else {
  ...
  return _createClass(MediaManagement, [{
    ...
    value: function handleMediaList() {
      ...
      _self.$body.off('click', '.js-media-list-title').on('click', '.js-media-list-title', function (event) {
        ...
        return false;
      }).on('click', '.js-up-one-level', function (event) {
        event.preventDefault();
